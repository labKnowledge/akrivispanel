...
export default function GithubDeploymentPage() {
  ...
  const [buildType, setBuildType] = useState<"auto" | "dockerfile" | "compose">(
    "auto",
  );
  ...
  const addPortBinding = () =>
    setPortBindings([...portBindings, { host: "", container: "" }]);
  const removePortBinding = (idx: number) =>
    setPortBindings(portBindings.filter((_, i) => i !== idx));
          ...
          chunk.split(/\n/).forEach((line) => {
            if (line.startsWith("data: ")) {
              ...
              if (
                ...
                logLine.toLowerCase().startsWith("error:")
              ) {
                errorMsg = logLine;
              }
            }
  ...
  return (
    <div className="w-full max-w-8xl mx-auto">
      <form
        className="space-y-4 bg-white shadow rounded p-6"
      ...
      >
        <div>
          ...
          <div className="text-xs text-gray-500 mt-1">
            Supports GitHub, Hugging Face Spaces, and any public Git provider.
            Paste the full URL (no .git required for Hugging Face).
          </div>
        </div>
        <div>
          ...
          {portBindings.map((port, idx) => (
            <div className="flex gap-2 mb-2" key={idx}>
              ...
              <input
                ...
                onChange={(e) =>
                  handlePortChange(idx, "container", e.target.value)
                }
                min={1}
                max={65535}
              />
      ...
      {(error || buildLogs.length > 0 || result) && (
        <div className="mt-8">
          ...
          {containerId &&
            buildLogs.some((l) =>
              l.toLowerCase().includes("deployment complete"),
            ) && (
              <button
      ...
      {result && result.status === "success" && (
        <div className="mt-8 bg-green-50 border border-green-200 rounded p-4">
          ...
          <div className="mb-2">
            You can now deploy this image as a container using the{" "}
            <b>Deploy Image</b> page or API.
          </div>
          <div className="text-xs text-gray-500">
            API Example:
            <br />
            <pre className="bg-gray-100 rounded p-2 mt-1">
              {JSON.stringify(result.nextStep, null, 2)}
            </pre>
          </div>
        </div>
      )}
