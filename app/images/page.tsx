...
export default function ImagesPage() {
  ...
  const handleDelete = async (img: DockerImage) => {
    const repoTag =
      img.RepoTags && img.RepoTags.length > 0 ? img.RepoTags[0] : undefined;
    const id = img.Id;
    ...
    <div className="w-full max-w-8xl mt-10 flex flex-col flex-1 px-2 space-between">
      ...
      {imagesLoading ? (
        <div className="flex flex-col items-center justify-center py-16">
          <svg
            className="animate-spin w-10 h-10 text-blue-500 mb-4"
            fill="none"
            viewBox="0 0 24 24"
          >
            <circle
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2  lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8 w-full">
          {images.map((img) => (
            <div
              key={img.Id}
              className="relative flex flex-col bg-white/70 backdrop-blur-lg rounded-2xl shadow-xl border-t-4 border-gradient-to-r from-blue-400 to-blue-600 p-6 hover:scale-[1.03] hover:shadow-2xl transition-all duration-200 min-h-[140px] group"
            >
              {/* Menu Icon */}
              <button
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <span
                  className="text-xl font-bold text-gray-800 truncate max-w-[120px]"
                  title={img.RepoTags?.join(", ") || "<none>"}
                >
                  {img.RepoTags?.[0] || "<none>"}
                </span>
                <span className="ml-auto px-2 py-0.5 rounded text-xs font-semibold bg-gradient-to-r from-blue-100 to-blue-300 text-blue-700 shadow">
                  {img.Id.slice(7, 19)}
                </span>
              </div>
              <div className="flex flex-wrap gap-2 items-center mb-3">
                <span className="text-xs px-2 py-0.5 rounded bg-gradient-to-r from-blue-50 to-blue-100 text-blue-700 font-semibold shadow">
                  {img.RepoTags?.join(", ") || "<none>"}
                </span>
                <span className="text-xs px-2 py-0.5 rounded bg-gray-100 text-gray-700 shadow">
                  {formatBytes(img.Size)}
                </span>
              </div>
              <div className="text-xs text-gray-400 mt-auto font-mono">
